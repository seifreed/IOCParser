...
class TestIOCRepoRetryLogic:
    def test_retry_path_finds_existing_row(self, tmp_path: Path) -> None:
        ...
        ioc = IOCModel(ioc_type="md5", value="direct_val", value_search="direct_val",
...
class TestSourceRepoRetryLogic:
    def test_retry_path_updates_metadata(self, tmp_path: Path) -> None:
        ...
        source = SourceModel(kind="file", value="direct.txt", value_search="direct.txt",
...
class TestHistoryRichImport:
    def _build_rich_export(self, tmp_path: Path) -> tuple[str, dict]:
        ...
        svc.persist_batch_job(source_kind="url", run_ids=(rid,),
            report={"total": 1, "successful": 1, "failed": 0, "items": [], "phase_timestamps": {"started_at": datetime.now(UTC).isoformat(), "finished_at": datetime.now(UTC).isoformat()}, "duration_ms": 100},
        ...
        dsvc.mark_completed(job_id=str(req.job_id), attempts=1, run_id=rid,
                            result_json={"ok": True}, metrics={"parse_ms": 10})
        ...
        dsvc.mark_dead_lettered(job_id=str(req2.job_id), attempts=3,
...
def test_json_renderer_with_warnings():
    ...
    r = ExtractionResult(
        ...
        warnings=(WarningMatch(ioc=IOC.from_raw("ips", "2.2.2.2"), warning_list="w", description="d"),),
    )
