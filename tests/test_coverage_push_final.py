...
def _db(tmp_path: Path, n: str = "t.db") -> str:
    uri = f"sqlite:///{tmp_path / n}"
    e = create_engine(uri, future=True)
    from iocparser.infrastructure.persistence_migration_runtime import migrate_engine
    migrate_engine(e)
    return uri


def _persist(uri: str, sk: str = "file", sv: str = "f.txt", iv: str = "x.com") -> int:
    ...
    def test_reraise_when_not_found(self, tmp_path: Path) -> None:
        ...
        with patch.object(s, "flush", side_effect=IntegrityError("x", {}, Exception())):
            with pytest.raises(IntegrityError):
                r._get_or_create(ioc_type="sha1", value="ghost", is_warning=False, warning_list="", warning_description="")
        s.close()
...
class TestHistoryImportFull:
    def _build_rich_db(self, tmp_path: Path) -> str:
        ...
        uri = _db(tmp_path, "rich.db")
        rid = _persist(uri, sk="url", sv="https://example.com/report", iv="evil.com")
        ...
        dsvc.mark_completed(job_id=str(req.job_id), attempts=1, run_id=rid, result_json={"ok": True}, metrics={"parse_ms": 10})
        return uri
    ...
    def test_full_export_import_with_url_sources_and_distributed_jobs(self, tmp_path: Path) -> None:
        ...
        uri2 = _db(tmp_path, "target_full.db")
...
class TestStreamingDuplicatePath:
    def test_process_multiple_streaming_duplicate_files(self, tmp_path: Path) -> None:
        ...
        results = process_multiple_files(
            [f, f], reader=reader, warning_service=None,
            request=MultiFileProcessingRequest(
                file_type=None, defang=False, check_warnings=False, force_update=False,
                include_types=(), exclude_types=(), streaming=True, chunk_size=1024 * 1024,
                overlap=128, max_workers=1,
            ),
        )
...
class TestUrlProcessingEdges:
    def test_retry_from_report_with_matches(self, tmp_path: Path) -> None:
        ...
        rp = tmp_path / "rr.json"
        rp.write_text(json.dumps({"items": [{"url": "https://a.com", "status": "failed", "retry_attempt": 1}]}))
        assert retry_attempt_for_url("https://a.com", str(rp), retry_batch_job=None, db_uri=None, occurrence=1) >= 1
