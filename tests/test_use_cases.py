...
def test_persist_run_logs_summary_on_success(caplog: pytest.LogCaptureFixture) -> None:
    unit_of_work = RecordingUnitOfWork()
    request = PersistRunInput(
        source=Source.from_raw("file", "/tmp/sample.txt"),
        result=ExtractionResult(
    ...
    with caplog.at_level("INFO", logger="iocparser.application.use_cases"):
        persist_run(request, unit_of_work=unit_of_work)

    messages = [record.getMessage() for record in caplog.records]
    assert any(
        "Persisted run 99" in message
        and "1 normal IOC(s)" in message
        and "1 warning IOC(s)" in message
