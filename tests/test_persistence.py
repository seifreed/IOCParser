...
def test_import_history_matches_existing_url_sources_without_normalized_url(tmp_path) -> None:
    db_path = tmp_path / "iocparser-import-url-source-no-normalized.db"
    service = SQLAlchemyPersistenceService(f"sqlite:///{db_path}")
    options = PersistOptions(
        defang=False,
        check_warnings=False,
...
def test_query_service_matches_legacy_spaced_non_url_source_values(tmp_path) -> None:
    ...
    assert service.query_runs_page(limit=10, source_value="sample.txt").total == 1
...
def test_query_service_does_not_wildcard_match_percent_in_legacy_url_source(tmp_path) -> None:
    ...
    service.persist_multiple_runs(
        [
            (
                "url",
                "https://example.test/report?x=100A25",
                ExtractionResult.from_grouped_payload({"domains": ["example.com"]}, {}),
            )
        ],
        tool_version="5.0.0",
